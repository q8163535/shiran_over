...
unsigned int transform_modbus( char * p_protocol_name)
{


#if 1

if (  NULL ==strstr( p_protocol_name,"csv" )  )
{
  return 0;
}
  ...
  * element, this will be the root element of the document. */
  ...
  char file_type_arr[10]   = { 0 } ;
  
  char data_type_arr[10] = { 0 } ;
  
    char data_id_arr[10] = { 0 } ;
  
  if ( ( NULL !=strstr ( csv_data_name,"3x" ) ) || ( NULL !=strstr ( csv_data_name,"3X" ) ) )
    {
      strcpy ( file_type_arr,"4" );
      strcpy ( data_type_arr,"R" );
      
            strcpy ( data_id_arr,"1" );
    }
  else    if ( ( NULL !=strstr ( csv_data_name,"4x" ) ) || ( NULL !=strstr ( csv_data_name,"4X" ) ) )
    {
      strcpy ( file_type_arr,"3" );
      strcpy ( data_type_arr,"RW" );
...
//         printf ( " headLine %d:  \n ", line_id );
          ...
          for ( row_n = 0; row_n < head_num; row_n++ )
            {
             ...
             printf ( " data_head_name[ %d ] %s   BAD_CAST  element %s   \n",row_n,data_head_name[ row_n ],   element );
	            ...
	            if ( 0 == strncmp ( data_head_name[ row_n ] ,"DataType",8 ) )
                {
                  rc = xmlTextWriterWriteAttribute ( writer, BAD_CAST  "DataLen",  BAD_CAST  element );
		  
   
                }
              else   if ( 0 == strncmp ( data_head_name[ row_n ] ,"DataSign",8 ) )
                {
               //    DataType="I" DataSign="U"
                  if ( element[0] == 'U' )
                    {
                      rc = xmlTextWriterWriteAttribute ( writer, BAD_CAST "DataType",  BAD_CAST  "I" );
                      rc = xmlTextWriterWriteAttribute ( writer, BAD_CAST "DataSign",  BAD_CAST  "U" );

                    }
                  else   if ( element[0] == 'F' )
                    {
                      rc = xmlTextWriterWriteAttribute ( writer, BAD_CAST "DataType",  BAD_CAST  "F" );
                      rc = xmlTextWriterWriteAttribute ( writer, BAD_CAST "DataSign",  BAD_CAST  "S" );
                    }
                  else if ( element[0] == 'S' )
                    {
                      rc = xmlTextWriterWriteAttribute ( writer, BAD_CAST "DataType",  BAD_CAST  "I" );
  ...
  free ( buf );
   ...
   rc = xmlTextWriterEndElement ( writer );  //end  table

  /* Close the element named HEADER. */
  // 停止写入HEADER结点
  rc = xmlTextWriterEndElement ( writer );  //end register 


  /* Close the element named FOOTER. */
  rc = xmlTextWriterEndElement ( writer );  //end protocol


  /* Close the element named FOOTER. */
  rc = xmlTextWriterEndElement ( writer );  //end root
...
int file_type =   get_measure_type (  p_protocol_name );
  ...
  while ( ( ent=readdir ( pDir ) ) !=NULL )
    {
	  ...
	  int csv_flag = -1;
	   ...
	   if(   get_measure_type ( ent->d_name ) >= 0 )
	  {
	    csv_flag = 1;
	  }
	  else  if ( ( NULL !=strstr (  ent->d_name,"3x" ) ) || ( NULL !=strstr ( ent->d_name,"4x" ) ) )
	  {
	     csv_flag = 2;
	  }
